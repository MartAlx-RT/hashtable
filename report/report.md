...
-   Время работы измерялось 
    ...
    (интринсик) `rdtsc` (подпись 'clocks' на графиках)
...
-   При профилировании программ использовался профилировщик `perf`
...
## Выводы
## Общая картина
Измерим время работы поиска для каждой хэш-функции.
...
## Детали
Рассмотрим последние три хэш-функции боле подробно.
...
Перепишем эту функцию, используя ассемблерную вставку:
```c
    asm goto volatile(
            "loop%=:"                                 "\n"
            "movq    (%[next], %[idx], 8), %[idx]"    "\n"
            "cmpq    (%[hashes], %[idx], 8), %[hash]" "\n"
            "jne    continue%="                       "\n"
...
В пределах погрешностей производительность функции не изменилась.
Поэтому, сгенерированный `gcc -O3` код является достаточно эффективным.
Откатимся на предыдущую версию.

Перепрофилируем программу, обратим внимание на неизвестную 
следущую за `tbl_find` неизвестную библиотечную функцию.

![perf disas](./images/asm_tbl_find.png)
...
Таким образом, `strcmp` занимает $14.53$% процессорного времени.
...
Далее используем выравнивание памяти и соответствующие инструкции для 
работы с выровненной памятью. Также попробуем убрать условный переход
(грузим всю строку всегда). Сравним производительность.

`key_cmp`, написанная на aligned-инструкциях
```c
	__m256i low1 = _mm256_load_si256((const __m256i *)key1);
	__m256i low2 = _mm256_load_si256((const __m256i *)key2);

	__m256i hi1 = _mm256_load_si256((const __m256i *)((uint64_t)key1 + 32));
	__m256i hi2 = _mm256_load_si256((const __m256i *)((uint64_t)key2 + 32));
...
#### Результаты измерений тактов
|               | unaligned mem|aligned mem  |aligned, no cmp   |
|    :---:      |    :---:     |    :---:    |    :---:         |  
| $t_\text{ср}$ |     1.09     |    1.07     |    1.05          |  
| $\sigma_{t}$  |     0.03     |    0.03     |    0.01          |  

Замечено небольшое ускорение. Кстати, компилятор встроил эту функцию
в код `tbl_find`
![inline cmp](./images/inline_cmp.png)

Попробуем теперь все поля структуры таблицы аллоцировать aligned-функциями.
...
Ускорения достигнуть не удалось. Интересно, что при использовании
aligned-функций уменьшилась погрешность измерения времени, что 
объясняется "более стабильной работой" с выровненной памятью.
